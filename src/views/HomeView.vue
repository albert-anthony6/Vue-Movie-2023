...
async function getShows(category: string, keyName?: string) {
  ...
  await showsStore
    .getShows(showType.value, category)
    .then((resp) => {
  ...
  <div class="home">
    ...
    <div v-if="shows.nowPlaying" class="now-playing">
      ...
      >
        <Slide v-for="slide in shows.nowPlaying.length" :key="slide">
          <div class="details">
            <h1 class="title">
              {{ shows.nowPlaying[slide - 1].title || shows.nowPlaying[slide - 1].name }}
            </h1>
            <p class="overview">
              {{ shows.nowPlaying[slide - 1].overview.slice(0, 303)
              }}<span v-if="shows.nowPlaying[slide - 1].overview.length > 303">...</span>
            </p>
            <RouterLink
              :to="`/show/${shows.nowPlaying[slide - 1].id}/?type=${route.query.type || 'movies'}`"
            >
          ...
          <img
            ...
            :src="`http://image.tmdb.org/t/p/${backdropSize}${
    ...
    <div v-if="shows.upcoming" class="upcoming">
      ...
      >
        <Slide v-for="slide in 10" :key="slide">
          <RouterLink
            :to="`/show/${shows.upcoming[slide].id}/?type=${route.query.type || 'movies'}`"
          >
            <img
    ...
    <div v-if="shows.popular" class="popular">
      ...
      >
        <Slide v-for="slide in 10" :key="slide">
          <RouterLink
            :to="`/show/${shows.popular[slide].id}/?type=${route.query.type || 'movies'}`"
          >
            <img
    ...
    <div v-if="shows.topRated" class="top-rated">
      ...
      >
        <Slide v-for="slide in 10" :key="slide">
          <RouterLink
            :to="`/show/${shows.topRated[slide].id}/?type=${route.query.type || 'movies'}`"
          >
            <img
        ...
        <template #addons>
          ...
          <Pagination />
          <RouterLink
            :to="`/category/top_rated/?type=${route.query.type || 'movies'}/?type=${
              route.query.type || 'movies'
            }`"
            class="view-all"
          >
            View All <font-awesome-icon class="fa-icon" icon="fa-solid fa-arrow-right" />
