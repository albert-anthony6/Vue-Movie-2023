  ...
  actions: {
    getShows: (showType: string, category: string, page = '1'): Promise<ShowCategoryData> =>
      new Promise((resolve, reject) => {
        axios
          .get(
            `/.netlify/functions/getShows/?showType=${showType}&category=${category}&page=${page}`
          )
          .then((resp) => {
            resolve(resp.data)
          })
    ...
    getShow: (showType: string, id: string): Promise<ShowDetails> =>
      new Promise((resolve, reject) => {
        axios
          .get(`/.netlify/functions/getShow/?id=${id}&showType=${showType}`)
          .then((resp) => {
            resolve(resp.data)
          })
          .catch((error) => {
    ...
    getVideos: (id: string): Promise<VideoData> =>
      new Promise((resolve, reject) => {
        axios
          .get(`/.netlify/functions/getVideos/?id=${id}`)
          .then((resp) => {
            // Only return one official trailer
            const trailer = resp.data.results.find(
              (video: VideoData) =>
                video.official === true && video.type === 'Trailer' && video.site === 'YouTube'
            )
            resolve(trailer)
          })
          .catch((error) => {
    ...
    getSearch: (showType: string, searchValue: string, page = '1'): Promise<ShowCategoryData> =>
      new Promise((resolve, reject) => {
        axios
          .get(
            `/.netlify/functions/getSearch/?showType=${showType}&searchValue=${searchValue}&page=${page}`
          )
          .then((resp) => {
            resolve(resp.data)
          })
